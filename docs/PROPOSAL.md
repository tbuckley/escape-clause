...
| `cancel_request({ticket} \| {tickets} \| {all: true}, reason?)` | Withdraws the agent's own pending tickets with no human involved — the one agent-invokable state transition, and strictly rejection-only: nothing executes, a cancelled ticket can never be approved afterwards, and relayed permission prompts are excluded. Ownership = the workspace the broker was launched from; with a shared store, another session's tickets are unreachable and indistinguishable from nonexistent ones. For cleaning up stale or superseded requests. |
